...
class WidgetManager():
    ...
    def __init__(self, wm_output=sys.stdout, customIconPath=None,
    ...
    def draw(self, widget):
        ...
        TODO: maybe cache values in a dict and build a string from those values
              instead of accessing each widget myvalue field each time.
        '''
        value = ""
...
class Widget():
    ...
    * If a widget wants its value changed, the self.value(newvalue) method
      should be called. The value in the PyBar is updated automatically.
    * If a widget needs to monitor or update a value periodically, then the
      widget should implement the thread() method.
    '''
    ...
    def toDzen2String(self, iconpath, iconcolor, textcolor):
        ...
        if icontext:
            icontext = "^fg(" + iconcolor + ")" + "^i(" + iconpath + \
                "/" + icontext + ".xbm)" + "^fg(" + textcolor + ")"
        ...
        if icontext and valuetext:
            return "{}{} {}{}".format(mouse_wrap_start, icontext, valuetext,
                                      mouse_wrap_end)
